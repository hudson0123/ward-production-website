            ...
            {packages.list.map((_: any, idx: number) => (
              <button
                ...
                className={`h-1 transition-all duration-300 ${
                  currentIndex === idx ? "w-8 bg-accent" : "w-1.5 bg-zinc-200"
                }`}
          ...
          {packages.list.map((_: any, idx: number) => (
            <button
              ...
              className={`h-1.5 transition-all duration-300 ${
                currentIndex === idx ? "w-12 bg-accent" : "w-4 bg-zinc-100 hover:bg-zinc-200"
              }`}
              style={{ borderRadius: '10px' }}
            />
          ))}
  ...
  return (
    <span
      ...
      style={{ 
        ...
        opacity: opacity,
        boxShadow: scale > 1.1 ? '0 10px 30px -10px rgba(var(--accent-rgb), 0.12)' : 'none'
      }}
    >
      {name}
    </span>
